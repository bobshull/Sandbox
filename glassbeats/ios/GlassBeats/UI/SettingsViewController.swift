...
final class SettingsViewController: UIViewController {
    ...
    private func makeVersionFooter() -> UIView {
        ...
        versionLabel.text = "GlassBeats \(version) (\(build))"
    ...
    private func sendFeedback() {
        ...
        let version = info?["CFBundleShortVersionString"] as? String ?? "?"
        let build   = info?["CFBundleVersion"] as? String ?? "?"
        let subject = "GlassBeats \(version) (\(build)) Feedback"

        let device = UIDevice.current
        var report = "GlassBeats \(version) (\(build))\nDate: \(Date())\nDevice: \(device.model) — iOS \(device.systemVersion)\n\n"
        ...
        if MFMailComposeViewController.canSendMail() {
            ...
            vc.setToRecipients(["bobby@glassbeats.app"])
            ...
            if let url = URL(string: "mailto:bobby@glassbeats.app?subject=\(encoded)") {
...
final class SyncHapticsPairCell: UITableViewCell {
    ...
    func configure() {
        ...
        cloudSwitch.isEnabled = PatternStore.cloudStatus != .unavailable
        if cloudSwitch.isOn { PatternStore.startCloudSync() }
        applyCloudState()
    ...
    private func applyCloudState() {
        switch PatternStore.cloudStatus {
        case .unavailable:
        ...
        case .notSynced:
            cloudIconView.image = UIImage(systemName: "exclamationmark.icloud")
            cloudIconView.tintColor = Theme.textFaint
            cloudStatusLabel.text = "Not synced"
        case .synced:
            ...
            cloudStatusLabel.text = "Synced"
        case .syncFailed:
            cloudIconView.image = UIImage(systemName: "exclamationmark.icloud")
            cloudIconView.tintColor = Theme.textFaint
            cloudStatusLabel.text = "iCloud sync unavailable"
        }
    }
