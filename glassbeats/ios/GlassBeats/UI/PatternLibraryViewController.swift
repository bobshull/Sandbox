...
final class PatternLibraryViewController: UIViewController,
    ...
    private let restoreBuiltInsButton = UIButton(type: .system)
    private var restoreButtonHeight: NSLayoutConstraint?
    ...
    override func viewDidLoad() {
        ...
        NSLayoutConstraint.activate([
            ...
            collectionView.bottomAnchor.constraint(equalTo: restoreBuiltInsButton.topAnchor, constant: -4),

            restoreBuiltInsButton.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 16),
            restoreBuiltInsButton.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -16),
            restoreBuiltInsButton.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -8),
        ...
        restoreButtonHeight = restoreBuiltInsButton.heightAnchor.constraint(equalToConstant: 34)
        restoreButtonHeight?.isActive = true
    ...
    private func buildHeader() -> UIView {
        ...
        nowPill.titleLabel?.lineBreakMode = .byTruncatingTail
        nowPill.setContentCompressionResistancePriority(.defaultLow, for: .horizontal)
        ...
        saveCfg.title = "Save Mix"
        saveCfg.image = UIImage(systemName: "plus.circle",
            withConfiguration: UIImage.SymbolConfiguration(pointSize: 12, weight: .semibold))
        saveCfg.imagePadding = 6
        saveCfg.contentInsets = NSDirectionalEdgeInsets(top: 6, leading: 12, bottom: 6, trailing: 12)
        ...
        saveCfg.background.strokeColor = ColorTheme.current.primaryColor.withAlphaComponent(0.55)
        ...
        NSLayoutConstraint.activate([
            ...
            saveBtn.leadingAnchor.constraint(greaterThanOrEqualTo: nowPill.trailingAnchor, constant: 10),
    ...
    private func configureRestoreButton() {
        ...
        cfg.title = "Restore built-in presets"
        cfg.image = UIImage(systemName: "arrow.counterclockwise")
        cfg.imagePadding = 6
        cfg.baseForegroundColor = ColorTheme.current.primaryColor
        ...
        restoreBuiltInsButton.configuration = cfg
        restoreBuiltInsButton.addTarget(self, action: #selector(restoreBuiltInsTapped), for: .touchUpInside)
        restoreBuiltInsButton.translatesAutoresizingMaskIntoConstraints = false
        view.addSubview(restoreBuiltInsButton)
    }
    ...
    @objc private func restoreBuiltInsTapped() {
        UserDefaults.standard.removeObject(forKey: Self.hiddenPresetsKey)
        reload()
    }

    func reload() {
        let hidden = hiddenPresetIds
        let hasHiddenPresets = !hidden.isEmpty
        restoreBuiltInsButton.isHidden = !hasHiddenPresets
        restoreButtonHeight?.constant = hasHiddenPresets ? 34 : 0
    ...
    @objc private func saveMixTapped() {
        dismiss(animated: true) { [weak self] in
            self?.delegate?.patternLibraryDidRequestSaveMix()
        }
    }
