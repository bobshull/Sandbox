...
final class AudioEngine {
    ...
    private let renderQueue = DispatchQueue(label: "glassbeats.audio.kitrender", qos: .userInitiated)
    ...
    private let schedulerQueue = DispatchQueue(label: "glassbeats.audio.scheduler", qos: .userInteractive)
    ...
    private func schedule(step: Int, atHostSeconds hostSeconds: Double) {
        ...
        for track in Tracks.all {
            ...
            if isAccented {
                let gain = AudioEngine.accentGains[track.id] ?? 1.25
                print("[GlassBeats] accent hit: \(track.id) step=\(step) gain=\(gain)×")
            }
            #endif
    ...
    static func exportFileName(patternName: String, format: ExportFormat) -> String {
        ...
        if base.isEmpty { base = "GlassBeats_Mix" }
